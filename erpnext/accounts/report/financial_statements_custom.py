...
def get_data(
    ...
    accounts = get_accounts(company, custom_type)
    ...
    # Add custom_type and custom_title
    if out and total:
        add_total_row(out, custom_type, balance_must_be, period_list, company_currency, custom_title)
...
# Initialize global variables to store each type totals
total_revenue = 0.0
total_cogs = 0.0
total_expense = 0.0
total_other = 0.0

def add_total_row(out, custom_type, balance_must_be, period_list, company_currency, custom_title, final=False):
    ...
    total_row = {
        "account_name": _(custom_title).format(_(custom_type), _(balance_must_be)),
        "account": _(custom_title).format(_(custom_type), _(balance_must_be)),
        "currency": company_currency,
        "opening_balance": 0.0,
    }
    ...
    for row in out:
        if not row.get("parent_account"):
            ...
            elif custom_type == 'Other':
                total_other = total_row["total"]
    ...
    if "total" in total_row:
        ...
        if custom_type == 'Cost of Good Sold':
            gross = {
                ...
                "currency": company_currency,
                "opening_balance": 0.0,
                "is_gross_profit": True  # Custom key to identify this row
            }

            for period in period_list:
                gross[period.key] = (total_revenue - total_cogs) if total_cogs else 0.0

            gross["total"] = total_revenue - total_cogs

            out.append(gross)
            # blank row after Gross Profit
            out.append({})
        
        if custom_type == 'Expense':
            operating_profit = {
                "account_name": _("Operating Profit"),
                "account": _("Operating Profit"),
                "currency": company_currency,
                "opening_balance": 0.0,
                "is_operating_profit": True  # Custom key to identify this row
            }

            for period in period_list:
                operating_profit[period.key] = operating_profit[period.key] = (total_revenue - total_cogs - total_expense) if total_expense else 0.0

            operating_profit["total"] = (total_revenue - total_cogs - total_expense)

            out.append(operating_profit)
            # blank row after Gross Profit
            out.append({})
    
    # Calculate and append percentage for each row
    for row in out:
        if row and "parent_account" in row:
            if custom_type == 'Income' and total_revenue:
                ...
                percentage = (row['total'] / total_revenue) * 100
            elif custom_type == 'Other' and total_other:
                percentage = (row['total'] / total_revenue) * 100
            else:
                percentage = 0.0
            ...
            if row["account_name"] in ["Total COGS", "Gross Profit", "Total Operational Expense", "Operating Profit", "Total Other Revenue & Expense", "Net Profit"]:
                percentage = (row['total'] / total_revenue) * 100
                row['percentage'] = f"{round(percentage, 2)}%"
            else:
                row['percentage'] = "###" 
    ...
    def compare_accounts(a, b):
        if re.split(r"\W+", a[key])[0].isdigit():
            ...
            if a.report_type != b.report_type and a.report_type == "Balance Sheet":
                return -1
            if a.custom_type != b.custom_type and a.custom_type == "Asset":
                return -1
            if a.custom_type == "Liability" and b.custom_type == "Equity":
                return -1
            if a.custom_type == "Income" and b.custom_type == "Expense":
                return -1
        else:
